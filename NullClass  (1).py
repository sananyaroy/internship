...
if is_time_allowed():
    ...
    final_html = wordcloud_html + "<p style='color:red;'>chart visible only between 5 PM and 5 PM IST.</p>"
with open("dashboard.html", "w", encoding='utf-8') as f:
    f.write(final_html)
        ...
        for i in range(1, len(category_df)):
            if category_df.loc[i, 'Significant_growth']:
                fig.add_trace(go.Scatter(
                    ...
                    y=[category_df.loc[i-1, 'Installs'], category_df.loc[i, 'Installs']],fill='tozeroy',mode='none',fillcolor='rgba(0, 100, 0, 0.3)',name='>20% Growth'))
    fig.update_layout(title='Monthly Installs with >20%',xaxis_title='Month',yaxis_title='Total Installs',legend=dict(itemsizing='constant'))
